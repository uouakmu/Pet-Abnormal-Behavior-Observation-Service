...
# ── Skin backbone 선택 ─────────────────────────────────────────────────────────
# True  → ViT-B/16  (미세 질감 패턴 — 피부 분류에 유리)
# False → EfficientNet-V2-S (기본, 학습 속도 우선)
USE_VIT_SKIN = False
...
print(f"🐶 Dog Abnormal Omni | Device: {DEVICE} | Skin backbone: {'ViT-B/16' if USE_VIT_SKIN else 'EfficientNet-V2-S'}")
...
DOG_EYES_CLASSES = [
    "dog_normal",
    "dog_결막염",
    "dog_궤양성각막질환",      # [v2] 상/하 통합
    "dog_비궤양성각막질환",    # [v2] 상/하 통합
    "dog_백내장",              # [v2] 초기/비성숙/성숙 통합 (라벨 불일치 확인)
    "dog_색소침착성각막염",
    "dog_안검내반증",
    "dog_안검염",
    "dog_안검종양",
...
# Eyes 유사 클래스 페널티
EYES_SIMILAR_GROUPS = [
    ["dog_궤양성각막질환", "dog_비궤양성각막질환"],
    ["dog_안검염", "dog_안검내반증"],
    ["dog_백내장", "dog_핵경화"],    # 수정체 혼탁 유사
]
...
class SkinModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.head = nn.Sequential(
            nn.Dropout(0.4), nn.Linear(feat, 256), nn.BatchNorm1d(256),
...
class EyesModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.head = nn.Sequential(
            nn.Dropout(0.4), nn.Linear(feat, 512), nn.BatchNorm1d(512),
...
# ─────────────────────────────── TRANSFORMS ───────────────────────────────────
# Skin: 피부색·질감 보존 → ColorJitter 최소화
...
# Eyes: 충혈·혼탁 색 변화 유지 → ColorJitter 유지
...
def _save_history_plot(history, best_acc):
    ...
    ax1.plot([h["skin_val"] for h in history], 'b-', linewidth=2)
    ax1.set_ylim(0, 1); ax1.set_title("Dog Skin Val Acc"); ax1.set_xlabel("Epoch"); ax1.grid(True, alpha=0.3)
    ax2.plot([h["eyes_val"] for h in history], 'r-', linewidth=2)
    ax2.set_ylim(0, 1); ax2.set_title("Dog Eyes Val Acc"); ax2.set_xlabel("Epoch"); ax2.grid(True, alpha=0.3)
    plt.suptitle(f"Dog Abnormal Omni | Best Avg {best_acc*100:.1f}%", fontweight="bold")
    plt.tight_layout()
    plt.savefig("dog_abnormal_omni_history.png", dpi=150)
    plt.close()

def val_loop(model, samples, transform):
    loader = make_loader(samples, transform, shuffle=False, drop_last=False)
    model.eval(); c, t = 0, 0
    with torch.no_grad():
...
def train():
    ...
    if _task_ready("eyes"):
        ...
        print("📦 Preparing eyes (dog)...")
        eyes_train, eyes_val = collect_copy_split(EYES_ROOT, "eyes", DOG_EYES_CLASSES)
    ...
    eyes_crit = WeightedSmoothLoss(DOG_EYES_CLASSES, similar_groups=EYES_SIMILAR_GROUPS, class_weights=eyes_w)

    # ── Models ─────────────────────────────────────────────────────────────────────
    skin_model = SkinModel(len(DOG_SKIN_CLASSES)).to(DEVICE)
    eyes_model = EyesModel(len(DOG_EYES_CLASSES)).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        # Validation
        skin_vacc = val_loop(skin_model, skin_val, TRANSFORM_VAL)
        eyes_vacc = val_loop(eyes_model, eyes_val, TRANSFORM_VAL)
